...
PointSplit::PointSplit(const Eigen::VectorXd & point)
  : split_point(point)
{
}
...
int PointSplit::getIndex(const Eigen::VectorXd & input) const
{
  ...
  if (input.rows() != split_point.rows()) {
    ...
    oss << "PointSplit::getIndex(): input has not enough rows: "
        << input.rows() << " rows while split_point has " << split_point.rows()
        << "rows";
