...
int LinearSplit::writeInternal(std::ostream& out) const
{
  ...
  bytes_written += starkit_utils::writeDoubleArray(out, coeffs.data(), dim);
  bytes_written += starkit_utils::write<double>(out, offset);
...
int LinearSplit::read(std::istream& in)
{
  ...
  bytes_read += starkit_utils::readDoubleArray(in, coeffs.data(), dim);
  bytes_read += starkit_utils::read<double>(in, &offset);
  return bytes_read;
}
