...
void FunctionApproximator::predict(const Eigen::VectorXd& input, int dim, double& mean, double& var) const
{
...
void FunctionApproximator::predict(const Eigen::VectorXd& input, double& mean, double& var) const
{
  ...
  if (output.rows() != 1)
  {
    ...
    oss << "FunctionApproximator::predict: requiring a 1D output to a " << output.rows() << "D output function";
...
void FunctionApproximator::check1DOutput(const std::string& caller_name) const
{
  ...
  {
    std::ostringstream oss;
    oss << "FunctionApproximator"
        << "::" << caller_name << ": "
        << "requiring a 1D output to a " << getOutputDim() << "D output function";
    throw std::logic_error(oss.str());
  }
}
