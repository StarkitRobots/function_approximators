...
Viewer::Viewer(const std::string& fa_path, const std::string& config_path, unsigned int width, unsigned int height)
  : rhoban_viewer::Viewer(width, height), dim_index(-1), sub_dim_index(-1)
{
...
void Viewer::updateStatus()
{
  ...
  rhoban_viewer::Viewer::updateStatus(oss.str());
}
...
bool Viewer::update()
{
  updateStatus();
  drawTiles();
  return rhoban_viewer::Viewer::update();
}
